...
source(sFile)
close(sFile)
udf_logmessage("Start running RCategoryPref.R.")

Args<-commandArgs()
v_s_filename<-basename(Args[6])
udf_logmessage(paste("Input file:", v_s_filename, sep=""))

setwd("/export/boss/xlm/data/categoryPref/in/")
udf_logmessage(paste("Read data from file:",v_s_filename,sep=""))
v_df_rawdata<-read.table(v_s_filename,header=FALSE,sep="\t",colClasses=c("character", "character", "integer", "integer", "integer", "integer", "integer"),col.names=c("category_cd", "gdt_openid", "itemview_cnt", "follow_items", "add_shopcart", "genorder_cnt", "payorder_cnt"), stringsAsFactors=FALSE)

udf_logmessage("Split input data by category.")
v_l_rawdata<-split(v_df_rawdata, v_df_rawdata$category_cd)
...
udf_logmessage(paste("The weight is as follows: ", str(v_l_retwgt), sep=""))
...
#写入文件
v_s_filename<-paste("/export/boss/xlm/data/categoryPref/out",v_s_filename,sep="/")
