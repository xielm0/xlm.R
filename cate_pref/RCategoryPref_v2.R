...
v_s_progname<-paste("RCategoryPref_v2 ", v_s_filename, sep="")
...
v_df_rawdata<-read.table(v_s_filename,header=FALSE,sep="\t",colClasses=c("character", "integer", "integer", "integer", "integer", "integer"),col.names=c("category_cd", "itemview_cnt", "follow_items", "add_shopcart", "genorder_cnt", "payorder_cnt"), stringsAsFactors=FALSE)

udf_logmessage_v2("Split input data by category.",v_s_progname)
...
#写入文件
setwd("/export/boss/xlm/data/categoryPref/out/")
...
udf_logmessage_v2("Done running RCategoryPref.R.",v_s_progname)
q()
